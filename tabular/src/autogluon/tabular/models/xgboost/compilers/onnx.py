...
class XGBoostOnnxCompiler:
    ...
    def compile(model, path: str, input_types=None):
        ...
        if input_types is None or not isinstance(input_types[0], tuple):
            raise RuntimeError("input_types argument should contain at least one tuple"
                               ", e.g. [((1, 14), np.float32)]")
        if isinstance(model, XGBoostOnnxPredictor):
            return model
        ...
        predictor = XGBoostOnnxPredictor(model=onnx_model)
        XGBoostOnnxCompiler.save(onnx_model, path)
        return predictor
    ...
    @staticmethod
    def load(path: str) -> XGBoostOnnxPredictor:
        """Load from the path that contains an onnx file."""
        import onnx
        onnx_bytes = onnx.load(path + "model.onnx")
        return XGBoostOnnxPredictor(model=onnx_bytes)
