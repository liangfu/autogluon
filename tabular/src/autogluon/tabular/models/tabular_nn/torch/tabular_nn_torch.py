...
class TabularNeuralNetTorchModel(AbstractNeuralNetworkModel):
    ...
    def _predict_tabular_data_tvm(self, new_data, process=True):
        ...
        if model.has_vector_features:
            num_features = new_data.data_list[0].shape[-1]
            input_shape = (self.max_batch_size, num_features)
            input_shapes.append(input_shape)
            data_batch.update(vector=[torch.randn(input_shape)])
        if model.has_embed_features:
            embed_groups = new_data.feature_groups['embed']
            assert(len(embed_groups) == len(new_data.num_categories_per_embedfeature))
            data_batch.update(embed=[])
            for i, g in enumerate(embed_groups):
                # num_features = new_data.num_categories_per_embedfeature[i]
